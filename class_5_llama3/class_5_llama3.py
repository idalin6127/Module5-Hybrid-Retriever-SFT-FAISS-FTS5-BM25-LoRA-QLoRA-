...
class BalancedResumeTrainer:
    ...
    def extract_key_resume_info(self):
        ...
        # Load resume
        resume = PyPDFLoader("C:\\AI_Coop\\Homework\\Week5\\class_5_llama3\\AI_Engineer_Ida_Lin_resume.pdf").load()
        all_docs = resume
        
        if os.path.exists("C:\\AI_Coop\\Homework\\Week5\\class_5_llama3\\portfolio_notes_Ida_Lin.txt"):
            portfolio = TextLoader("C:\\AI_Coop\\Homework\\Week5\\class_5_llama3\\portfolio_notes_Ida_Lin.txt").load()
    ...
    def create_focused_training_data(self, full_resume_text, chunks):
        ...
        for i, chunk in enumerate(key_chunks):
            ...
            prompt = f"""
                You are an interviewer. Based on the following resume section, generate 3 sets of **high-quality** interview Q&A.

                    Requirements:
                    - Answers must be in **first person**; use only the information from the resume section and given facts.
                    - Do not fabricate companies, dates, or numbers; if information is missing, reply with "Not mentioned in the resume section."
                    - Each answer must include specific technologies, tools, timeframes, scale, or metrics (if present).
                    - Output must be strictly formatted as JSON:
    ...
    },
    {
        ...
        # Add identity pairs 2 times (balanced repetition)
        for _ in range(2):
